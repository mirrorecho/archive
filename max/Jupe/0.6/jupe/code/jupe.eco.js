...
// NEXT UP: USE GLOBAL JUPE OBJECT (jupe.myfun.ly.json, jupe.myfun.din.json)
...
function remove(libraryname, name) {
	jupe.library(libraryname).remove(name);
}
...
function get_valuefilename(libraryname, name, fileExtension, purpose) {
	if (purpose) outlet(1, [purpose, libraryname, name, jupe.library(libraryname).getValueFileName(name, fileExtension)]);
	else outlet(0, [libraryname, name, jupe.library(libraryname).getValueFileName(name, fileExtension)]);
}

function load_value(libraryname, name, fileExtension) {
	jupe.library(libraryname).loadValue(name, fileExtension);
}

function save_value(libraryname, name, fileExtension, clear) {
	jupe.library(libraryname).saveValue(name, fileExtension);
	if (clear) jupe.library(libraryname).remove(name);	
}

function save_ifnew_value(libraryname, name, fileExtension, defaultValue) {
	if (defaultValue) jupe.library(libraryname).set(name, defaultValue);
	if (!jupe.library(libraryname).valueFileExists(name, fileExtension)) jupe.library(libraryname).saveValue(name, fileExtension);
}

function contains(libraryname, name, purpose) {
	if (purpose) outlet(1, [purpose, libraryname, name, jupe.library(libraryname).contains(name)]);
	else outlet(0, [libraryname, name, jupe.library(libraryname).contains(name)]);
}
...
jupe._library_type = function(libraryName) {
	...
	this.valueFileExists = function(name, fileExtension) {
		var f = new File(this.getValueFileName(name, fileExtension));
		if (f.isopen) {	//if succeed in opening file
			f.close();
			return true;
		} else { //file doesn’t exist
	...
	this.remove = function(name) {
		if (this.contains(name)) this.dict.remove(name);
	}

	this.set = function(name, value) {
		if (value) this.dict.set(name, value);
		else {
			alert_note("new dict: " + name);
			this.dict.set(name, new Dict(name));
		}
	...
	this.getValueFileName = function(name, fileExtension) {
		if (!fileExtension) fileExtension = "txt";
		return jupe.library("eco").get("workroot") + "other/" + this.longName + "." + name.replace(/::/g, ".") + "." + fileExtension;
	}

	this.loadValue = function(name, fileExtension) {
		this.set(name, jupe.readFile(this.getValueFileName(name, fileExtension), true))
	}


	this.saveValue = function(name, fileExtension) {
		jupe.writeFile(this.getValueFileName(name, fileExtension), this.get(name));
	}
...
jupe.readFile = function(filePath, ignoreNotFound) {
	myString = "";
	maxChars = 2000;
	...
	if (f.isopen) {
		...
		if (!ignoreNotFound) alert_error("Error reading file: " + filePath);
	}
	return myString;
}

jupe.writeFile = function(filePath, content) {
	maxChars = 2000;
	var f = new File(filePath,"write", "TEXT");
	if (f.isopen) {
		f.writestring(content);
		f.close();
		alert_note("Saved file: " + filePath);
	} else {
		alert_error("Error reading file: " + filePath);
	}
}
